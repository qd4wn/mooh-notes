...
`lsh-cx` 是一个很适合学习 Shell 原理和 C 工程结构的小项目。

它的重点不在于功能复杂，而在于把 shell 最核心的几件事拆清楚：

- 如何读取用户输入
- 如何把输入解析成参数数组
- 为什么 builtin 要在当前进程执行
- 外部命令为什么需要 `fork + execvp + waitpid`
- C 项目中如何管理每一轮命令产生的动态资源

理解这个项目之后，再去看 Bash、Zsh 这类完整 shell，就能更容易抓住它们的底层执行模型。
